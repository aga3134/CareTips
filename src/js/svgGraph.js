...
svgGraph.PieChart = function(param){
	...
	var w = graph.width()-padL-padR, h = graph.height()-padT-padB;
	var svg = d3.select(param.selector);
	svg.selectAll("*").remove();

	var g = svg.append("g")
		.attr("class","group")
		.attr("transform", "translate(" + (w*0.5+padL) + "," + (h*0.5+padT) + ")");
...
svgGraph.MultiDegree = function(param){
	...
	for(var j=0;j<=param.data.length;j++){
		...
		var ptX = r*Math.cos(angle);
		var ptY = r*Math.sin(angle);
		bgOutline.append("line")
			.attr("x1",0).attr("y1",0)
			.attr("x2",ptX).attr("y2",ptY)
			.attr("fill","none")
			.attr("stroke","#888888");
	}
	...
	for(var j=0;j<param.data.length;j++){
		...
		var ptX = r*Math.cos(angle);
		var ptY = r*Math.sin(angle);
		fgShape.append("circle")
			.attr("cx",ptX).attr("cy",ptY).attr("r",5)
			.attr("fill",color(j))
			.attr("opacity",0.8)
			.attr("stroke","none")
			.attr("data-info",param.infoFn(param.data[j]))
			.on("mouseover",function(d){
				var cur = d3.select(this);
				$(param.textInfo).text(cur.attr("data-info"));
			});
